...
export default function SessionDetailPage() {
  ...
  // Check if session can be recorded (not completed or cancelled, not template)
  const isTemplate = session.isTemplate || !session.week;
  const canRecord = !isTemplate && (session.status === 'PLANNED' || session.status === 'IN_PROGRESS');
  ...
  return (
    <div className="space-y-6">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {se.exercise.videoUrl && (
                        <button
                          onClick={() => setVideoModal({ name: se.exercise.name, url: se.exercise.videoUrl! })}
                          className="rounded-full p-1 text-teal-600 hover:bg-teal-50 hover:text-teal-700 transition"
                          title={t('watchExerciseVideo')}
                        >
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-3">
                      <div className="flex items-center gap-1.5">
                        ...
                        <input
                          ...
                          className="w-14 rounded border border-slate-200 dark:border-slate-700 px-2 py-1 text-center text-sm"
                          onBlur={(e) => {
      ...
      {videoModal && (
        <>
          <div className="fixed inset-0 z-50 bg-black/60" onClick={() => setVideoModal(null)} />
            ...
            <div className="relative w-full max-w-3xl rounded-xl bg-white dark:bg-slate-800 shadow-2xl overflow-hidden">
              <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-700 px-5 py-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-semibold text-slate-900 dark:text-white">{videoModal.name}</h3>
                </div>
                <button
