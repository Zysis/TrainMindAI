/**
 * TrainMind — Seed per la GUIDA UTENTE (org AV)
 *
 * Copia di seed-team14.ts con quattro differenze: punta all'organizzazione AV,
 * genera due stagioni (2025/26 conclusa + 2026/27 in corso) così che dashboard,
 * ACWR e analisi abbiano dati recenti, sposta le date dei test entro oggi e
 * lascia un infortunio aperto con RTP attivo.
 *
 * ATTENZIONE: come l'originale, CANCELLA squadre, atleti, wellness, piani,
 * test e infortuni esistenti dell'organizzazione prima di ricreare tutto.
 *
 * Run: pnpm --filter @trainmind/db exec tsx prisma/seed-guida.ts
 *
 ...
 *
 * Run originale: pnpm --filter @trainmind/db exec tsx prisma/seed-team14.ts
 */
...
const MEASURE_DATES = [
  new Date('2025-09-08'), // Test d'ingresso pre-season
  new Date('2025-11-10'), // Fine preparazione
  new Date('2026-01-12'), // Metà andata
  new Date('2026-03-16'), // Metà ritorno
  new Date('2026-06-15'), // Chiusura stagione 2025/26
  new Date('2026-08-24'), // Ingresso stagione 2026/27
];
...
/* La guida ha bisogno di dati vivi: la stagione 2025/26 conclusa fornisce
   storico e trend, la 2026/27 (stessa struttura spostata di un anno) mette
   sessioni completate nelle ultime settimane e sessioni pianificate avanti. */
const bumpName = (n: string) =>
  n.includes('2025/26') ? n.replace('2025/26', '2026/27') : n.replace('2026', '2027');

const PERI_CONFIGS_ALL: PeriCfg[] = [
  ...PERI_CONFIGS,
  ...PERI_CONFIGS.map((c) => ({
    ...c,
    name: bumpName(c.name),
    start: addDays(new Date(c.start), 365).toISOString().slice(0, 10),
  })),
];
...
async function seedOrg(orgName: string) {
  ...
  const team = await prisma.team.create({
    data: {
      name: 'Prima Squadra',
      description: 'Prima squadra senior — stagione 2026/27',
      color: '#ef4444',
  ...
  // 2. Giocatori
  const roster = ROSTERS[orgName] ?? ROSTERS.MM;
  ...
  for (const cfg of PERI_CONFIGS_ALL) {
  ...
  for (const cfg of INJURY_CONFIGS) {
    ...
    const roster14 = (ROSTERS[orgName] ?? ROSTERS.MM)[cfg.athleteIdx];
