...
import { Bell, Search, ChevronDown, CheckCheck, AlertTriangle, Info, XCircle, CheckCircle2, Settings, LogOut, Menu as MenuIcon } from 'lucide-react';
...
export function Topbar() {
  ...
  return (
    <header className={cn(
      "fixed right-0 top-0 z-30 flex h-[var(--topbar-height)] items-center justify-between border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 sm:px-4 md:px-6 transition-[left] duration-300 dark:border-slate-700 dark:bg-slate-900",
      // Mobile: no left offset (drawer overlays). md+: align next to sidebar.
      'left-0',
      collapsed ? 'md:left-[var(--sidebar-collapsed-width)]' : 'md:left-[var(--sidebar-width)]',
      // Safe-area on notch / dynamic island
      ...
      <button
        type="button"
        onClick={openMobile}
        className="md:hidden inline-flex h-11 w-11 items-center justify-center rounded-lg text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800"
        aria-label="Open menu"
      >
        <MenuIcon className="h-5 w-5" />
      </button>
      {/* Search (hidden on small screens to save space) */}
      <div className="hidden sm:flex w-full max-w-md items-center gap-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 px-3 py-2 dark:border-slate-700 dark:bg-slate-800">
      ...
      <div className="flex items-center gap-2 sm:gap-4 ml-auto">
        {/* Plan badge (START / PRO / ULTRA) — visibile da sm in su per non saturare topbar mobile */}
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          {/* Dropdown */}
          {showDropdown && (
            <div className="fixed sm:absolute right-2 sm:right-0 top-[calc(var(--topbar-height)+8px)] sm:top-full sm:mt-2 w-[calc(100vw-1rem)] sm:w-96 max-w-md rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-xl dark:border-slate-700 dark:bg-slate-800">
              {/* Header */}
