...
// Mobile build: if NEXT_PUBLIC_API_URL is explicitly empty string ('' in .env.local),
// API_BASE_URL stays empty and apiFetch produces relative URLs like /api/v1/...
// Those are then proxied to API_INTERNAL_URL by next.config.mjs rewrites (no CORS).
// If the env var is undefined (not set at all), fall back to localhost:3001 for safety.
